...
def test_page_exceeds_maximum(client, db_session, seed_database):
    ...
    assert (
        response_data["detail"] == "No movies found."
...
def test_movie_list_with_pagination(client, db_session, seed_database):
    ...
    expected_movies = (
        db_session.query(MovieModel).offset(offset).limit(per_page).all()
    )
    expected_movie_ids = [movie.id for movie in expected_movies]
    returned_movie_ids = [movie["id"] for movie in response_data["movies"]]

    assert (
        expected_movie_ids == returned_movie_ids
    ), "Movies on the page mismatch."
...
def test_get_movie_by_id_valid(client, db_session, seed_database):
    min_id = (
        db_session.query(MovieModel.id)
    ...
    assert response_data["date"] == str(
        expected_movie.date
    ), "Returned date does not match the expected date."
    assert (
        response_data["score"] == expected_movie.score
    ), "Returned score does not match the expected score."
    assert (
        response_data["genre"] == expected_movie.genre
    ), "Returned genre does not match the expected genre."
    assert response_data["overview"] == expected_movie.overview, (
        "Returned overview does not match the expected " "overview."
    )
    assert (
        response_data["crew"] == expected_movie.crew
    ), "Returned crew does not match the expected crew."
    assert response_data["orig_title"] == expected_movie.orig_title, (
        "Returned original title does not match the " "expected title."
    )
    assert (
        response_data["status"] == expected_movie.status
    ), "Returned status does not match the expected status."
    assert response_data["orig_lang"] == expected_movie.orig_lang, (
        "Returned original language does not match the " "expected language."
    )
    assert response_data["budget"] == float(expected_movie.budget), (
        "Returned budget does not match the " "expected budget."
    )
    assert (
        response_data["revenue"] == expected_movie.revenue
    ), "Returned revenue does not match the expected revenue."
    assert (
        response_data["country"] == expected_movie.country
    ), "Returned country does not match the expected country."
